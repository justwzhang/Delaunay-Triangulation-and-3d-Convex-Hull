...
int CConvexHull::_volume_sign(CConvexHullMesh::CFace* f, const CPoint& p) 
{
    ...
    CPoint B1 = v[0]->point() - p;
    CPoint B2 = v[1]->point() - p;
    CPoint B3 = v[2]->point() - p;
    double V = (1.0 / 6.0) * (B1 * (B2 ^ B3));
    if (V > 0) return 1;
    if (V == 0) return 0;
    return -1;
}
...
{ 
    ...
    {
        M::CFace* pF = *fiter;
        if (_volume_sign(pF, p)!= 1) return false;
    }
    return true;
}
...
void CConvexHull::_remove_visiable(const CPoint& p) 
{
    ...
    {
        M::CFace* pF = *fiter;
        if (_volume_sign(pF, p) != 1) visiable_faces.push_back(pF);
    }
...
void CConvexHull::_close_cap(const CPoint& p)
{
    ...
    for (auto pD : loop->halfedges())
    {
        ...
        M::CFace* pF = m_pMesh.insert_face(idx, ++m_max_face_id);
        m_pMesh.compute_normal(pF);
    }
}
